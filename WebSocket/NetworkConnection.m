...
//    _notifications = [NotificationsDataSource sharedInstance];
    ...
    _parserMap = @{
                   ...
                   @"header": ^(IRCCloudJSONObject *object) {
                       ...
                       _currentCount = 0;
                       NSLog(@"idle interval: %f clock offset: %f stream id: %@", _idleInterval, _clockOffset, _streamId);
                       if(_accrued > 0) {
                           [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                   ...
                   @"oob_include": ^(IRCCloudJSONObject *object) {
                       ...
                       _reconnectTimestamp = -1;
                       NSLog(@"oob_include, invalidating BIDs");
                       [_buffers invalidate];
...
-(void)_scheduleTimedoutBuffers {
    for(Buffer *buffer in [_buffers getBuffers]) {
        if(buffer.timeout > 0) {
            ...
            NSLog(@"Requesting backlog for buffer: %@", buffer.name);
